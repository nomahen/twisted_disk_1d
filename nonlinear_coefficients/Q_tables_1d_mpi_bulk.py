...
print "my rank is: ", rank, " and the total size is: ", size, "\n\n"
...
p.add_argument('-2d',"--dim",dest="dim",type=int,default=0, help = "Whether to produce 1D or 2D table. 0 ~ 1D, 1 ~ 2D. Currently only 1D is supported.")
args = p.parse_args()
...
Q1_send = np.zeros(npts)
Q2_send = np.zeros(npts)
Q3_send = np.zeros(npts)

if (rank == 0):
    Q1_recv = np.zeros(npts)
    Q2_recv = np.zeros(npts)
    Q3_recv = np.zeros(npts)
else:
    Q1_recv = None
    Q2_recv = None
    Q3_recv = None
...
# I think here we want to make sure npts is a multiple over our number of threads
perrank = npts//size
comm.Barrier()
...
ic_guess = []
r = 1 # Temporary, as I have not added support for 2D yet
